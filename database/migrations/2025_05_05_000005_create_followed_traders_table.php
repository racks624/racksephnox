...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('followed_traders')) {
            Schema::create('followed_traders', function (Blueprint $table) {
                ...
                $table->decimal('copy_ratio', 5, 2)->default(100);
                $table->boolean('auto_copy')->default(true);
                $table->decimal('max_copy_amount', 15, 2)->nullable();
                $table->timestamps();
                $table->unique(['follower_id', 'trader_id']);
            });
        }
    }
    public function down() { Schema::dropIfExists('followed_traders'); }
};
