<nav x-data="{ open: false, scrolled: false, lotteryOpen: false, currencyOpen: false }" x-init="() => { window.addEventListener('scroll', () => { scrolled = window.scrollY > 10 }); }"
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <!-- Lottery Dropdown -->
                    <div class="relative" x-data="{ lotteryOpen: false }">
                        <button @click="lotteryOpen = !lotteryOpen" class="inline-flex items-center px-3 py-2 text-sm font-medium text-ivory hover:text-gold focus:outline-none transition">
                            <i class="fas fa-dice-d6 mr-1"></i> Lottery
                            <svg class="ml-1 h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/>
                            </svg>
                        </button>
                        <div x-show="lotteryOpen" @click.away="lotteryOpen = false" class="absolute left-0 mt-2 w-56 rounded-md shadow-lg bg-cosmic-deep/95 backdrop-blur border border-gold/30 z-50">
                            <a href="{{ route('lottery.index') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-play mr-2"></i> Play Lottery</a>
                            <a href="{{ route('lottery.tournaments') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-trophy mr-2"></i> Tournaments</a>
                            <a href="{{ route('lottery.missions') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-tasks mr-2"></i> Daily Missions</a>
                            <a href="{{ route('bonus-wheel.index') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-cog mr-2"></i> Bonus Wheel</a>
                            <a href="{{ route('lottery.social') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-users mr-2"></i> Live Wins</a>
                            <a href="{{ route('lottery.history') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-history mr-2"></i> History</a>
                            <a href="{{ route('lottery.leaderboard') }}" class="block px-4 py-2 text-sm text-ivory hover:bg-gold/10"><i class="fas fa-chart-simple mr-2"></i> Leaderboard</a>
                        </div>
                    </div>
                    @endauth
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                <!-- User Dropdown with Currency Switcher -->
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <div class="px-3 py-1 text-xs text-gold-400">Currency</div>
                        <div class="grid grid-cols-2 gap-1 px-2 py-1">
                            <button onclick="setCurrency('KES')" class="text-left px-2 py-1 text-sm hover:bg-gold/10 rounded">KES</button>
                            <button onclick="setCurrency('USD')" class="text-left px-2 py-1 text-sm hover:bg-gold/10 rounded">USD</button>
                            <button onclick="setCurrency('EUR')" class="text-left px-2 py-1 text-sm hover:bg-gold/10 rounded">EUR</button>
                            <button onclick="setCurrency('BTC')" class="text-left px-2 py-1 text-sm hover:bg-gold/10 rounded">BTC</button>
                            <button onclick="setCurrency('ETH')" class="text-left px-2 py-1 text-sm hover:bg-gold/10 rounded">ETH</button>
                            <button onclick="setCurrency('USDT')" class="text-left px-2 py-1 text-sm hover:bg-gold/10 rounded">USDT</button>
                        </div>
                        <div class="border-t border-gold/20 my-1"></div>
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden bg-cosmic-deep/95 backdrop-blur-md border-t border-gold/20">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Mobile Lottery Dropdown (expanded list) -->
            <div class="pl-6 space-y-1">
                <div class="text-xs text-gold-400 mt-2 mb-1">Lottery</div>
                <x-responsive-nav-link :href="route('lottery.index')" class="text-gold-400">
                    <i class="fas fa-play mr-2 w-5"></i> Play Lottery
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('lottery.tournaments')" class="text-gold-400">
                    <i class="fas fa-trophy mr-2 w-5"></i> Tournaments
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('lottery.missions')" class="text-gold-400">
                    <i class="fas fa-tasks mr-2 w-5"></i> Daily Missions
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('bonus-wheel.index')" class="text-gold-400">
                    <i class="fas fa-cog mr-2 w-5"></i> Bonus Wheel
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('lottery.social')" class="text-gold-400">
                    <i class="fas fa-users mr-2 w-5"></i> Live Wins
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('lottery.history')" class="text-gold-400">
                    <i class="fas fa-history mr-2 w-5"></i> History
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('lottery.leaderboard')" class="text-gold-400">
                    <i class="fas fa-chart-simple mr-2 w-5"></i> Leaderboard
                </x-responsive-nav-link>
            </div>
        </div>
...
if ('requestIdleCallback' in window) {
    requestIdleCallback(() => {
        const links = ['{{ route("dashboard") }}', '{{ route("machines.index") }}', '{{ route("trading.index") }}', '{{ route("wallet") }}', '{{ route("lottery.index") }}', '{{ route("lottery.tournaments") }}', '{{ route("bonus-wheel.index") }}'];
        links.forEach(url => {
